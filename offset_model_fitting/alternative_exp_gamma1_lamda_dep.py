...
## mis-centering fraction has a lambda dependence
import numpy as np
...
def make_model(r_offset, rlambda, lmd):
       # remember to adjust the prior ranges if the posterior values are out of range.
       rho_0_0=Uniform('Rho0_0', lower=0.3, upper=1 )
       rho_0_alpha=Uniform('Rho0_alpha', lower=0.01, upper=0.5 )
       r0=Uniform('R0', lower=0.0001, upper=0.1 )
       tau=Uniform('tau', lower=0.04, upper=0.5 )

       rho_0=rho_0_0*(lmd/80.0)**rho_0_alpha
...
if __name__ == "__main__":
 ...
 for ii in np.arange(0, len(lmd_bins)):
    ...
    mcmc_file='mcmc_output/XCS_exp_gamma1_lambda_dep_lmd%i_lt%i'%(lmd_min, lmd_max)
    M=pymc.Model(make_model(r_offset, rlmd, lmd))
    ...
    rho_0=np.loadtxt(mcmc_file+'/Chain_0/Rho0_0.txt')
    rho_alpha=np.loadtxt(mcmc_file+'/Chain_0/Rho0_alpha.txt')
    r0=np.loadtxt(mcmc_file+'/Chain_0/R0.txt')
    tau=np.loadtxt(mcmc_file+'/Chain_0/tau.txt')
    print 'Chain Results rho0, rho_alpha, sigma, tau: %0.3f +- %0.3f, %0.3f +- %0.3f, %0.3f+-%0.3f, %0.4f+-%0.4f'%(np.mean(rho_0), np.std(rho_0), np.mean(rho_alpha), np.std(rho_alpha), np.mean(r0), np.std(r0), np.mean(tau), np.std(tau))
