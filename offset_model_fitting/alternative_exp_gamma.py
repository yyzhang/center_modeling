...
if __name__ == "__main__":
 ...
 for ii in np.arange(0, len(lmd_bins)):
    ...
    mcmc_file='mcmc_output/XCS_exp_gamma_lmd%i_lt%i'%(lmd_min, lmd_max)
    ...
    k=np.loadtxt(mcmc_file+'/Chain_0/k.txt')
    print 'Chain Results rho, sigma, tau, k: %0.3f +- %0.3f, %0.3f+-%0.3f, %0.4f+-%0.4f, %0.3f+-%0.3f'%(np.mean(rho_0), np.std(rho_0), np.mean(r0), np.std(r0), np.mean(tau), np.std(tau), np.mean(k), np.std(k))
